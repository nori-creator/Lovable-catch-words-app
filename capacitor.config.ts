 ...
 *
 * ## 切り替えは「ここを消すだけ」では**ない**
 * サーバー関数の呼び先はビルド時に**相対パス**として焼き込まれるので、
 * 同梱すると `capacitor://localhost/_serverFn/…` を叩きに行って、
 * AIも図鑑も復習も全部動かなくなる。呼び先の付け替えとサーバー側の
 * CORS が要る。手順と、いまどこまで済んでいるかは
 * **docs/capacitor-bundling.md** に書いてある。
 * (付け替えの仕組みは実装済み・既定では無効。CORS と実機確認が未了。)
 *
 * ## 表示先の切り替え
