  ...
  .handler(async ({ data }) => {
    ...
    const prompt =
      data.targetLanguage === "zh-TW"
        ? "この画像から、台湾華語の学習対象として有用な名詞を5つ選んで返してください。台湾教育部準拠の正式な繁体字で。注音と拼音、日本語の意味、カテゴリを含めること。"
        : `この画像から、${data.targetLanguage}の学習対象として有用な名詞を5つ選び、headword(${data.targetLanguage})、日本語の意味、カテゴリを返してください。`;
    ...
    // ai v6: structured output is in result.experimental_output
