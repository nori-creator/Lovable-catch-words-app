...
/**
 * 行き先が現れるのを待つ上限。これを過ぎたら諦める（本物は必ず出す）。
 *
 * 400ms では足りなかった。面の中身は手元の一覧を種にして即座に組むが、
 * 写真そのものは端末の控えから取り出す（IndexedDB → `blob:`）ので、
 * **遅い端末では見出しの箱が出るまでに数百ms**かかる。諦めると飛ばずに
 * 切り替わるだけになる — 直そうとしている「スムーズでない」がそのまま残る。
 * 1秒待つ。待っている間は押した札の写しが出たままなので、画面は空にならない。
 */
const FIND_TIMEOUT_MS = 1000;
