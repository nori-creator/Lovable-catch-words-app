 ...
 *
 * ## 見出しは付けない（オーナー決定 2026-09-15「今のままで見出しなくていい」）
 *
 * ここには長いあいだ「軸ごとに見出しを付けて束ねる」と書いてあったが、
 * **実際の描画はずっと1列だった** — 2026-09-13 に main 側で見出しが外れた
 * まま、注釈だけが古い設計を語り続けていた。注釈が実物と違うのは、
 * 無い注釈より悪い。決まった形を書く。
 *
 * ・軸（限定／どこで／いつ／どんな場面で／どんな物か／どんな気持ちで）で
 *   **束ねる計算は生きている**（`lib/scene-bubbles.ts`、試験付き）
 * ・ただし**見出しは出さない**。軸ごとに全候補を並べるとスマホで何段にも
 *   膨らみ、その下にある意味と例文が遠くなる
 * ・束は**どの札を先に出すかの順**として効く。`AXIS_ORDER` が
 *   限定 → どこで → いつ → どんな場面で → どんな物か → どんな気持ちで の順
 *   なので、下の「先頭4件」は**この優先順で選ばれた4件**であって、
 *   たまたま先に来た4件ではない。
 *
 * **束ねる計算を消してはいけない。** 見出しを出していないからといって
 * 外すと、4件の選び方がただの登場順に落ちる。
 *
...
export function SceneBubbles({
  ...
  // スマホで何段にも膨らむため、重複を除いた先頭4件に絞る。
  // **順は `AXIS_ORDER` のまま**なので、限定やどこで の札が先に残る。
  const items = groups
