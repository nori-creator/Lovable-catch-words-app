...
export function ScanAnalyzing_v3crystal({ stage }: { stage: Stage }) {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      resize();
      if (reduced) calmRedrawRef.current?.();
    };
    ...
    const accent =
      hour >= 17 && hour < 20
        ? [230, 210, 235] // dusk warm-silver
        : hour >= 20 || hour < 5
          ? [200, 220, 255] // night cool-silver
    ...
    if (reduced) {
      const drawCalm = () => {
        ...
        const veil =
          stageRef.current === "matching" ? 0.3 : stageRef.current === "reading" ? 0.22 : 0.12;
    ...
    const tick = (now: number) => {
      ...
      const veil = st === "matching" ? 0.3 : st === "reading" ? 0.22 : 0.12;
      ...
      if (st === "reading" && Math.random() < 0.05) {
        spawnRipple(
          Math.random() * w,
          Math.random() * h,
          Math.min(w, h) * (0.14 + Math.random() * 0.14),
          1200,
        );
      }
