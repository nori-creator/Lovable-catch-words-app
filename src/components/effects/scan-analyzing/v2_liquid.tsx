...
export function ScanAnalyzing_v2liquid({ stage }: { stage: Stage }) {
  const t = useT();
  ...
  useEffect(() => {
    ...
    const accent =
      hour >= 5 && hour < 11
        ? [180, 240, 255] // dawn cyan
        : hour >= 11 && hour < 17
          ? [125, 211, 252] // day cyan
          : hour >= 17 && hour < 20
            ? [255, 190, 200] // dusk warm
    ...
    const tick = (now: number) => {
      ...
      if (st === "sensing") {
        ...
        for (const l of locks) {
          ...
          ctx.lineTo(x, y);
          ctx.lineTo(x + bl, y);
          ctx.moveTo(x + cw - bl, y);
          ctx.lineTo(x + cw, y);
          ctx.lineTo(x + cw, y + bl);
          ctx.moveTo(x + cw, y + ch - bl);
          ctx.lineTo(x + cw, y + ch);
          ctx.lineTo(x + cw - bl, y + ch);
          ctx.moveTo(x + bl, y + ch);
          ctx.lineTo(x, y + ch);
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-6 flex flex-col items-center gap-2">
        ...
        <p className="font-mono-tight text-caption tracking-[0.32em] text-white/85 uppercase">
          {label}
        </p>
      </div>
    </div>
